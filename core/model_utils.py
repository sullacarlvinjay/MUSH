...
try:
    ...
    from .models.mock_ml import tf, np, mock_analyze_mushroom
    ML_AVAILABLE = False
    logging.warning("ML dependencies not available, using mock functions")

# Suppress TensorFlow logging only if TensorFlow is available
if ML_AVAILABLE:
...
class TFLiteModel:
    ...
    def predict(self, input_data):
        ...
        if scale and scale != 0.0:
            ...
            if hasattr(np, 'issubdtype') and np.issubdtype(input_detail['dtype'], np.integer):
        ...
        if out_scale and out_scale != 0.0:
            output_data = out_scale * (output_data.astype('float32') - out_zero_point)
        return output_data
...
# Verify model files exist only if ML is available
if ML_AVAILABLE:
...
def get_mushroom_detector_model() -> TFLiteModel:
    ...
    if not ML_AVAILABLE:
        from .models.mock_ml import mock_get_mushroom_detector_model
        return mock_get_mushroom_detector_model()
...
def get_edibility_model() -> TFLiteModel:
    ...
    if not ML_AVAILABLE:
        from .models.mock_ml import mock_get_edibility_model
        return mock_get_edibility_model()
...
def get_species_model() -> TFLiteModel:
    ...
    if not ML_AVAILABLE:
        from .models.mock_ml import mock_get_species_model
        return mock_get_species_model()
...
def analyze_mushroom_image(image: Image.Image) -> Dict[str, Any]:
...
def analyze_mushroom(image_path: str) -> Dict[str, Any]:
    """Main function to analyze mushroom image with fallback to mock."""
    if not ML_AVAILABLE:
        logger.warning("Using mock analysis - ML dependencies not available")
        return mock_analyze_mushroom(image_path)
    
    try:
        # Use the real analysis function
        with Image.open(image_path) as image:
            return analyze_mushroom_image(image)
    except Exception as e:
        logger.error(f"Real analysis failed, falling back to mock: {str(e)}")
        return mock_analyze_mushroom(image_path)