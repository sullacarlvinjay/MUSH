    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if settings.EMAIL_HOST_USER and settings.EMAIL_HOST_PASSWORD:
                ...
                subject = f'{getattr(settings, "EMAIL_SUBJECT_PREFIX", "[MushGuard] ")} Verify your MushGuard account'
                message = f'Welcome to MushGuard! Please verify your email by clicking this link: {verify_url}'
                from_email = settings.DEFAULT_FROM_EMAIL
                recipient_list = [user.email]
                
                try:
                    # Try EmailJS first (bypasses SMTP blocking)
                    import sys
                    import os
                    sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
                    from emailjs_service import send_email_via_emailjs
                    
                    logger.info(f"Attempting EmailJS with service_id: {os.getenv('EMAILJS_SERVICE_ID')}")
                    logger.info(f"Template ID: {os.getenv('EMAILJS_TEMPLATE_ID')}")
                    logger.info(f"Public Key: {os.getenv('EMAILJS_PUBLIC_KEY')}")
                    
                    emailjs_result = send_email_via_emailjs(
                        to_email=user.email,
                        subject=subject,
                        message=message,
                    ...
                    if emailjs_result:
                        logger.info(f"Verification email sent via EmailJS to {user.email}")
                    else:
                        logger.error("EmailJS failed, falling back to console backend")
                        # Fallback to console backend
                        result = send_mail(subject, message, from_email, recipient_list, fail_silently=True)
                        if result:
                            logger.info(f"Verification email sent via console to {user.email}")
                        else:
                            logger.error(f"Failed to send verification email to {user.email}")
                            
                except ImportError as e:
                    logger.error(f"EmailJS import failed: {str(e)} - using console backend")
                    # EmailJS not available, use console backend
                    result = send_mail(subject, message, from_email, recipient_list, fail_silently=True)
                    if result:
                        logger.info(f"Verification email sent via console to {user.email}")
                    else:
                    ...
                    logger.error(f"EmailJS error: {str(e)} - falling back to console")
                    # Never crash the app due to email issues
            else:
                logger.warning(f'Email credentials not configured. Cannot send verification email to {user.email}')
                # Show clear message to user
                messages.error(request, 'Email service is not configured. Please contact administrator to set up email verification.')
